...
const defaultOptions: TimeChartOptionsBase = {
...
type TPluginStates<TPlugins> = {
    [P in keyof TPlugins]: TPlugins[P] extends TimeChartPlugin<infer TState>
        ? TState
        : never;
};

function completeSeriesOptions(
    s: Partial<TimeChartSeriesOptions>
): TimeChartSeriesOptions {
    s.data = s.data
        ? DataPointsBuffer._from_array(s.data)
        : new DataPointsBuffer();
...
export class TimeChart<TPlugins extends TimeChartPlugins = NoPlugin> {
...
:host {
    ...
    position: relative;
        ...
        this.contentBoxDetector = new ContentBoxDetector(
            el,
            this.model,
            coreOptions
        );
        this.nearestPoint = new NearestPointModel(
            this.canvasLayer,
            this.model,
            coreOptions,
            this.contentBoxDetector
        );
        this._options = coreOptions;

        this.plugins = Object.fromEntries(
            Object.entries(options?.plugins ?? {}).map(([name, p]) => [
                name,
                p.apply(this),
            ])
